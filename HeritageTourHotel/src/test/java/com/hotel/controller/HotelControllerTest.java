...
public class HotelControllerTest {
	...
	public void setup() {
		...
		// test cases on addHotel
		@Test
		public void addHotelControllerTest() throws Exception 
		{
			when(cs.addHotel(any())).thenReturn(hotel);
			mockMvc.perform(post("/hotel/ad/addhotel")
					.contentType(MediaType.APPLICATION_JSON)
					.content(asJSONString(hotel)))
					.andExpect(status().isCreated());
			verify(cs,times(1)).addHotel(any());
		}
		...
		public void getAllHotelsControllerTest1() throws Exception
		{
			...
			mockMvc.perform(MockMvcRequestBuilders.get("/hotel/ad/getallhotels")
			.contentType(MediaType.APPLICATION_JSON)
			.content(asJSONString(hotel)))
